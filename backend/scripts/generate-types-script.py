...
def generate_typescript_interface(class_name: str, fields: List[Tuple[str, str, bool]]) -> str:
...
def main():
    ...
    print()
    print("export function formatDate(date: Date): string {")
    print("  return date.toISOString().split('T')[0];")
    print("}")
    print()
    print("export function formatTime(date: Date): string {")
    print("  return date.toTimeString().split(' ')[0];")
    print("}")
    print()
    print("export function parseTime(timeStr: string): Date {")
