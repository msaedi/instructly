...
def _get_or_create_seed_subcategory(session: Session) -> ServiceSubcategory:
    ...
    if category is None:
        category = ServiceCategory(
            ...
            description="Catalog services generated by reset_and_seed_database_enhanced.py",
        )
...
def _service_min_hourly_rate(service: Service) -> float:
...
def generate_instructor_summary(session: Session):
    ...
    for service, user in services_with_instructors:
        skill = _service_name(service)
        skill_summary[skill]["count"] += 1
        price = _service_min_hourly_rate(service)
        skill_summary[skill]["min_price"] = min(skill_summary[skill]["min_price"], price)
        skill_summary[skill]["max_price"] = max(skill_summary[skill]["max_price"], price)
    ...
    for template in INSTRUCTOR_TEMPLATES:
        ...
        if service_to_soft_delete and service_to_soft_delete in services:
            ...
            for i in range(random.randint(3, 5)):
                ...
                booking_time_fields = _build_booking_time_fields(booking_date, start_time, end_time)
                booking = Booking(
                    ...
                    total_price=Decimal(str(booking_rate * duration_minutes / 60)),
    ...
    for template in INSTRUCTOR_TEMPLATES:
        ...
        if service_to_soft_delete and "_services" in template:
            ...
            if service and service.is_active:
                ...
                has_bookings = (
                    session.query(Booking)
                    .filter(Booking.instructor_service_id == service.id)
                    .first()
                    is not None
                )
                ...
                if has_bookings:
                    ...
                    logger.info(f"Soft deleted service: {_service_name(service)} (ID: {service.id})")
