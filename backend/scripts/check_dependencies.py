            ...
            try:
                ...
                with open(py_file, 'r') as f2:
                    ...
                    if '__init__' in content:
                        for service in ['availability_service', 'booking_service', 
                                      'conflict_checker', 'slot_manager', 
                                      'notification_service', 'week_operation_service',
                                      'bulk_operation_service', 'presentation_service']:
...
def generate_dependency_report(dependencies: Dict[str, Set[str]], cycles: List[List[str]]):
    ...
    print("-"*40)
    for service, dependents in sorted(reverse_deps.items(), 
