...
def upgrade():
    ...
    op.execute("CREATE TYPE day_of_week AS ENUM ('monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday')")
    
    # Create availability_windows table
    op.create_table('availability_windows',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('instructor_id', sa.Integer(), nullable=False),
        sa.Column('day_of_week', sa.Enum('monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday', name='day_of_week', create_constraint=False, native_enum=False), nullable=True),
        ...
        sa.CheckConstraint('(day_of_week IS NOT NULL AND specific_date IS NULL) OR (day_of_week IS NULL AND specific_date IS NOT NULL)', name='check_day_or_date'),
        sa.CheckConstraint('end_time > start_time', name='check_time_order')
    )
    
    op.create_index(op.f('ix_availability_windows_instructor_id'), 'availability_windows', ['instructor_id'], unique=False)
    op.create_index(op.f('ix_availability_windows_day_of_week'), 'availability_windows', ['day_of_week'], unique=False)
    op.create_index(op.f('ix_availability_windows_specific_date'), 'availability_windows', ['specific_date'], unique=False)
    ...
    op.create_index(op.f('ix_blackout_dates_instructor_id'), 'blackout_dates', ['instructor_id'], unique=False)
