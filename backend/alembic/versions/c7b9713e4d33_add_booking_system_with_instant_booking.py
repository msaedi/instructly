...
def upgrade() -> None:
    ...
    op.create_index('idx_bookings_instructor_date_status', 'bookings', 
                    ['instructor_id', 'booking_date', 'status'])
    op.create_index('idx_bookings_student_status', 'bookings', 
                    ['student_id', 'status'])
    ...
    op.add_column('availability_slots', 
                  sa.Column('booking_id', sa.Integer(), nullable=True))
    op.create_foreign_key('fk_availability_slots_booking', 
                          'availability_slots', 'bookings', 
                          ['booking_id'], ['id'])
    op.create_index('idx_availability_slots_booking_id', 
                    'availability_slots', ['booking_id'])
    
    # Add instructor settings
    op.add_column('instructor_profiles',
                  sa.Column('min_advance_booking_hours', sa.Integer(), 
                           nullable=False, server_default='2'))
    op.add_column('instructor_profiles',
                  sa.Column('buffer_time_minutes', sa.Integer(), 
                           nullable=False, server_default='0'))
    op.add_column('instructor_profiles',
                  sa.Column('auto_accept_bookings', sa.Boolean(), 
