...
class TestPublicAvailabilityIntegration:
    ...
    async def test_full_availability_flow(
        ...
        # PENDING bookings should hold the slot. Without an explicit location_type,
        # public availability now uses the conservative travel default, so the
        # 09:00-10:00 booking blocks until 11:00.
        available_slots = result["availability_by_date"][tomorrow_str]["available_slots"]
