...
def test_seed_location_aliases_falls_back_to_ambiguous_for_multiple_display_groups(tmp_path) -> None:
    engine = _setup_engine()
    payload = {
        "region_code": "nyc",
        "city_id": "city-1",
        "aliases": [
            {
                "alias": "harbor district",
                "region_name": "Harbor District",
                "type": "colloquial",
                "confidence": 1.0,
            }
        ],
    ...
    with engine.begin() as conn:
        conn.execute(
            text(
                ...
                INSERT INTO region_boundaries (id, region_type, region_name, display_name, display_key)
                VALUES
                    ('r1', 'nyc', 'Harbor District-East', 'Harbor East', 'nyc-harbor-east'),
                    ('r2', 'nyc', 'Harbor District-West', 'Harbor West', 'nyc-harbor-west')
                """
            )
        )
    ...
    with Session(engine) as session:
        row = session.execute(
            ...
            {"city_id": "city-1", "alias": "harbor district"},
        ).mappings().one()

    assert row["region_boundary_id"] is None
    assert bool(row["requires_clarification"]) is True
    assert json.loads(str(row["candidate_region_ids"])) == ["r1", "r2"]
