...
def ensure_instructor_service_for_tests(
    ...
    *,
    ...
    # Ensure we have a category + subcategory (3-level taxonomy)
    subcategory_id = catalog_fields.pop("subcategory_id", None)
    if subcategory_id is None:
        category = (
            session.query(ServiceCategory)
            .filter(ServiceCategory.name == "Testing Services")
            .one_or_none()
        )
        if not category:
