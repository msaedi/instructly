...
class StripeIdentityMixin(BaseService):
    ...
    def get_latest_identity_status(self, user_id: str) -> Dict[str, Any]:
        """Return the latest Stripe Identity verification status for a user."""
        stripe_sdk = _stripe_service_module().stripe
        try:
            self._check_stripe_configured()
            sessions = stripe_sdk.identity.VerificationSession.list(limit=20)
            latest = None
            for session in sessions.get("data", []):
                try:
                    metadata = getattr(session, "metadata", {}) or {}
                    if str(metadata.get("user_id")) == str(user_id):
                        if latest is None or getattr(session, "created", 0) > getattr(
                            latest, "created", 0
                        ):
                            latest = session
                except Exception:
            ...
            self.logger.error("Stripe error getting identity status: %s", exc)
            raise ServiceException(f"Failed to get identity status: {exc}")
        except Exception as exc:
            if isinstance(exc, ServiceException):
                raise
            self.logger.error("Error getting identity status: %s", exc)
