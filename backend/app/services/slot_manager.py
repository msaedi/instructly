...
class SlotManager(BaseService):
    ...
    def create_slot(
        ...
        availability = self.db.query(InstructorAvailability).filter(
    ...
    def update_slot(
        ...
        self.logger.info(
            f"Updated slot {slot_id}: {new_start}-{new_end}"
        )
    ...
    def delete_slot(
        ...
        remaining_slots = self.db.query(AvailabilitySlot).filter(
    ...
    def merge_overlapping_slots(
        ...
        if preserve_booked:
            ...
            if booked_slots:
                self.logger.info(
                    f"Preserving {len(booked_slots)} booked slots"
                )
    ...
    def _has_bookings_on_date(
        ...
        return self.db.query(Booking).join(
            AvailabilitySlot, 
            Booking.availability_slot_id == AvailabilitySlot.id
        ).filter(
