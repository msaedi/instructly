...
import { AnalyticsNav } from '@/app/(admin)/admin/analytics/AnalyticsNav';
              ...
              {!result ? (
                ...
                <div className="space-y-3 text-sm">
                  <div className="grid grid-cols-[110px_1fr_auto] items-center gap-2">
                    ...
                    <button onClick={() => { navigator.clipboard.writeText(result.code); const btn = event?.currentTarget as HTMLButtonElement | undefined; if (btn) { const original = btn.textContent; btn.textContent = 'Copied'; setTimeout(() => { btn.textContent = original || 'Copy'; }, 1200); } }} className="rounded-full px-2 py-1 text-xs ring-1 ring-gray-300/70 dark:ring-gray-700/60 hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer">Copy</button>
                  </div>
                  <div className="grid grid-cols-[110px_1fr_auto] items-center gap-2">
                    ...
                    <button onClick={(event) => { navigator.clipboard.writeText(result.join_url); const btn = event.currentTarget; const original = btn.textContent; btn.textContent = 'Copied'; setTimeout(() => { btn.textContent = original || 'Copy'; }, 1200); }} className="rounded-full px-2 py-1 text-xs ring-1 ring-gray-300/70 dark:ring-gray-700/60 hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer">Copy</button>
                  </div>
                  <div className="grid grid-cols-[110px_1fr_auto] items-center gap-2">
                    ...
                    <button onClick={(event) => { navigator.clipboard.writeText(result.welcome_url); const btn = event.currentTarget; const original = btn.textContent; btn.textContent = 'Copied'; setTimeout(() => { btn.textContent = original || 'Copy'; }, 1200); }} className="rounded-full px-2 py-1 text-xs ring-1 ring-gray-300/70 dark:ring-gray-700/60 hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer">Copy</button>
                  </div>
                </div>
              )}
            </div>
