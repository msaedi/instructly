        ...
        {!requires2FA ? (
          <form className="space-y-6" onSubmit={handleSubmit} noValidate>
            ...
            <div>
              ...
              <div className="mt-1">
                <input id="email" name="email" type="email" autoComplete="email" required value={formData.email} onChange={handleChange} disabled={isSubmitting} className="appearance-none block w-full px-3 py-2 h-10 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-purple-500 focus:border-purple-500 disabled:bg-gray-100 disabled:cursor-not-allowed bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 autofill-fix" placeholder="you@example.com" />
            ...
            <div className="flex items-center justify-between">
              <div className="text-sm">
                <Link href="/forgot-password" className="font-medium text-purple-700 hover:text-purple-800 transition-colors" onClick={() => logger.debug('Navigating to forgot password')}>
                  Forgot password
                </Link>
              </div>
            </div>
          ...
          <form className="space-y-6" onSubmit={handleVerify2FA}>
            ...
            <div>
              ...
              <input id="twofa" inputMode="numeric" pattern="[0-9]*" maxLength={6} className="mt-1 block w-full px-3 py-2 h-10 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-purple-500 focus:border-purple-500" value={twoFactorCode} onChange={(e) => setTwoFactorCode(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !isVerifying2FA && twoFactorCode.trim().length >= 6) { e.preventDefault(); handleVerify2FA(); } }} placeholder="123 456" />
            </div>
            <div>
              <label htmlFor="backup" className="block text-sm font-medium text-gray-700">Backup code (optional)</label>
              <input id="backup" className="mt-1 block w-full px-3 py-2 h-10 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-purple-500 focus:border-purple-500" value={backupCode} onChange={(e) => setBackupCode(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !isVerifying2FA && backupCode.trim().length > 0) { e.preventDefault(); handleVerify2FA(); } }} placeholder="ABCD-EFGH-1234" />
            </div>
