              ...
              <button
                ...
                className={`block w-full text-left px-3 py-2 rounded-md cursor-pointer transition-colors ${
                  hovered === option.value && option.value !== ''
                    ? 'bg-purple-50 text-(--color-brand)'
                    : ''
                } ${
                  value === option.value && option.value !== ''
                    ? 'bg-purple-100 text-(--color-brand) ring-1 ring-inset ring-[#D4B5F0] font-semibold'
        ...
        <SectionHeroCard
          ...
          actions={
            <button
              ...
              className="inline-flex items-center gap-1 p-2 rounded-md text-(--color-brand) hover:bg-purple-50 dark:hover:bg-purple-900/30 transition-colors"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 mb-8">
          <div className="insta-dashboard-stat-card rounded-md sm:rounded-lg p-4 sm:p-6 h-32 sm:h-40">
            <div className="flex items-start justify-between gap-4 h-full">
              ...
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-purple-100 flex items-center justify-center shrink-0">
                <Wallet className="w-5 h-5 sm:w-6 sm:h-6 text-(--color-brand)" />
              </div>
            </div>
          </div>
          <div className="insta-dashboard-stat-card rounded-md sm:rounded-lg p-4 sm:p-6 h-32 sm:h-40">
          ...
          <div className="insta-dashboard-stat-card rounded-md sm:rounded-lg p-4 sm:p-6 h-32 sm:h-40">
            <div className="flex items-start justify-between gap-4 h-full">
              ...
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-purple-100 flex items-center justify-center shrink-0">
                <BookOpen className="w-5 h-5 sm:w-6 sm:h-6 text-(--color-brand)" />
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="p-2 rounded-md text-(--color-brand) hover:bg-purple-50 dark:hover:bg-purple-900/30 transition-colors"
                >
                      ...
                      {invoices.map((invoice) => {
                        ...
                        return (
                          <tr key={`${invoice.booking_id}-${invoice.created_at}`}>
                            ...
                            <td className="py-3 pr-4 font-semibold text-(--color-brand)">{formatCents(invoice.instructor_share_cents)}</td>
                    ...
                    {payoutsData.payouts.map((payout) => {
                      ...
                      return (
                        <tr key={payout.id}>
                          ...
                          <td className="py-3 pr-4 font-semibold text-(--color-brand)">{formatCents(payout.amount_cents)}</td>
                          <td className="py-3 pr-4">
                            <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${statusColor}`}>
        ...
        <div className="px-5 py-4">
          ...
          <div className="mt-4 flex justify-end">
            <button
              ...
              className="inline-flex items-center justify-center h-10 px-4 rounded-lg bg-(--color-brand) text-white font-semibold disabled:opacity-50 insta-primary-btn"
            >
