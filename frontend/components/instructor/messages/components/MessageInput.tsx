...
export function MessageInput({
  ...
  if (messageDisplay !== 'inbox') {
    return (
      <div className="p-6 border-t border-gray-200 dark:border-gray-700 text-sm text-gray-500 dark:text-gray-400">
    ...
    <div className="p-4 border-t border-gray-200 dark:border-gray-700 space-y-3">
      ...
      {pendingAttachments.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {pendingAttachments.map((file, index) => (
            <span
              key={`${file.name}-${index}`}
              className="inline-flex items-center gap-2 rounded-full bg-purple-50 border border-purple-200 px-3 py-1 text-xs text-(--color-brand)"
            >
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="h-10 w-10 flex items-center justify-center bg-(--color-brand) text-white rounded-full hover:bg-[#5f1aa4] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
