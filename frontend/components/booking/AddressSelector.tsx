...
export function AddressSelector({
  ...
  if (isLoading) {
    return <div className="text-sm text-gray-500 dark:text-gray-400">Loading addresses...</div>;
  }
      ...
      {hasAddresses ? (
        <div className="space-y-2">
          ...
          <button
            ...
            className="text-sm text-(--color-brand) hover:text-purple-900 dark:hover:text-purple-300 hover:underline"
          >
  ...
  return (
    <button
      ...
      className={`w-full p-3 text-left border rounded-lg transition-colors ${
        isSelected ? 'border-(--color-brand) bg-purple-50' : 'border-gray-200 dark:border-gray-700'
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          ...
          {address.is_default && (
            <span className="ml-2 text-xs bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded">Default</span>
          )}
        </div>
        {!hasCoords && (
