...
const hasWildcardHundredMsMediaOrigin = hundredMsMediaOrigins.some(
  (origin) => origin.includes('*') && origin.endsWith('.100ms.live'),
);
const normalizedHundredMsMediaOrigins = Array.from(
  new Set(
    hasWildcardHundredMsMediaOrigin
      ? [...hundredMsMediaOrigins, 'https://100ms.live']
      : hundredMsMediaOrigins,
  ),
);
const hundredMsImageOrigins = normalizedHundredMsMediaOrigins;
...
const mediaSrcOrigins = ["'self'", 'blob:', ...normalizedHundredMsMediaOrigins];
...
const cspPolicyValue = [
  ...
  `img-src 'self' data: blob: https://assets.instainstru.com https://*.cloudflare.com https://*.r2.cloudflarestorage.com https://*.stripe.com https://*.tile.jawg.io https://*.basemaps.cartocdn.com https://cdn.jsdelivr.net ${hundredMsImageOrigins.join(' ')}`,
