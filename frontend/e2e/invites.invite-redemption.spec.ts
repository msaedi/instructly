...
const BETA_BASE =
  process.env.E2E_BETA_BASE_URL || 'http://beta-local.instainstru.com:3100';
...
test.describe('beta invite redemption across origins', () => {
  test('generate invite on preview and redeem on beta host', async ({ page, browser }) => {
    ...
    const adminBase = test.info().project.use?.baseURL ?? 'http://localhost:3000';
    await page.goto(new URL('/admin/beta/invites', adminBase).toString(), { waitUntil: 'networkidle' });
