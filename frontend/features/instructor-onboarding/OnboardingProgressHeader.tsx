...
const STEP_DEFS = [
  ...
  { key: 'skills', label: 'Add Skills', href: '/instructor/onboarding/skill-selection' },
  { key: 'verification', label: 'Verify Identity', href: '/instructor/onboarding/verification' },
  { key: 'payment', label: 'Payment Setup', href: '/instructor/onboarding/payment-setup' },
] as const;
...
export function OnboardingProgressHeader({ activeStep, stepStatus, walkerVariant = 'walk' }: Props) {
  ...
  const updateWalker = useCallback(() => {
    ...
    const targetIndex = STEP_DEFS.findIndex((step) => step.key === activeStep);
    const target = targetIndex >= 0 ? stepButtonRefs.current[targetIndex] : null;
  ...
  const walkerClassName = walkerVariant === 'bounce' ? 'inst-anim-walk-profile' : 'inst-anim-walk';
  const armClass = walkerVariant === 'bounce' ? 'inst-anim-leftArm-fast' : 'inst-anim-leftArm';
  const armRightClass = walkerVariant === 'bounce' ? 'inst-anim-rightArm-fast' : 'inst-anim-rightArm';
  const legLeftClass = walkerVariant === 'bounce' ? 'inst-anim-leftLeg-fast' : 'inst-anim-leftLeg';
  const legRightClass = walkerVariant === 'bounce' ? 'inst-anim-rightLeg-fast' : 'inst-anim-rightLeg';
  ...
  return (
    <header className="bg-white backdrop-blur-sm border-b border-gray-200 px-4 sm:px-6 py-4">
      <div className="flex items-center justify-between max-w-full relative">
        ...
        >
          <div className={`absolute ${walkerClassName}`} style={{ top: '-12px', left: `${walkerLeft}px` }}>
