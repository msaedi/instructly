        ...
        {onBookingUpdate && (
          <button
            onClick={() =>
              onBookingUpdate((prev) => ({
                ...prev,
                duration: 'invalid-duration' as unknown as number,
              }))
            }
          >
            Invalidate Duration
...
jest.mock('../PaymentProcessing', () => {
  return function MockPaymentProcessing({
    ...
    latestPaymentProcessingProps = { amount, bookingType };
...
jest.mock('../PaymentSuccess', () => {
  return function MockPaymentSuccess({
    ...
    latestPaymentSuccessProps = { confirmationNumber, ...(cardLast4 ? { cardLast4 } : {}) };
...
describe('PaymentSection', () => {
  ...
  describe('targeted merge and quote fallbacks', () => {
    it('keeps existing booking fields when refreshed order data is blank or invalid', async () => {
      ...
      fetchBookingDetailsMock.mockResolvedValue({
        id: '',
        instructor_id: '',
        instructor: { first_name: '', last_initial: null },
        service_name: '',
        booking_date: null,
        start_time: '',
        end_time: '',
        ...
        total_price: 'still-not-a-number',
        meeting_location: '',
      ...
      await waitFor(() => {
        expect(latestPaymentConfirmationProps?.booking).toMatchObject({
          bookingId: 'booking-123',
          instructorId: 'instructor-456',
          instructorName: 'John D.',
          lessonType: 'Piano',
    ...
    it('builds a student-location quote payload when booking location and metadata hints are blank', async () => {
      let capturedQuotePayload: Record<string, unknown> | null = null;

      usePricingPreviewControllerMock.mockImplementation(({ quotePayload }) => {
        capturedQuotePayload = quotePayload as Record<string, unknown> | null;
        return {
          preview: {
            base_price_cents: 10000,
            student_fee_cents: 1500,
    ...
    it('forwards earliest credit expiry to confirmation in the step flow', async () => {
      useCreditsMock.mockReturnValue({
        data: { available: 50, expires_at: '2026-12-31T00:00:00Z' },
        isLoading: false,
        refetch: jest.fn(),
      });
      ...
      await waitFor(() => {
        expect(latestPaymentConfirmationProps?.creditEarliestExpiry).toBe('2026-12-31T00:00:00Z');
      });
    });

    it('forwards earliest credit expiry to confirmation in the inline flow', async () => {
      useCreditsMock.mockReturnValue({
        data: { available: 50, expires_at: '2027-01-15T00:00:00Z' },
        isLoading: false,
        refetch: jest.fn(),
      });
    ...
    it('falls back to zero amount in the processing step when booking total is missing and no preview is available', async () => {
      usePricingPreviewControllerMock.mockReturnValue({
        preview: null,
        error: null,
        loading: false,
    ...
    it('skips checkout entirely when booking totals are missing and no preview is available', async () => {
      ...
      await waitFor(() => {
        expect(onSuccess).toHaveBeenCalledWith('Bcredits-only-booking');
      });

      expect(paymentServiceMock.createCheckout).not.toHaveBeenCalled();
    });

    it('reuses the original booking start time and location when in-memory edits clear them', async () => {
      let capturedQuotePayload: Record<string, unknown> | null = null;
      ...
      fireEvent.click(screen.getByText('Clear Start Time'));
    ...
    it('derives duration from the original end time when the draft clears its end time', async () => {
      let capturedQuotePayload: Record<string, unknown> | null = null;
      ...
      usePricingPreviewControllerMock.mockImplementation(({ quotePayload }) => {
        ...
        return {
          preview: {
            ...
            student_fee_cents: 2250,
            student_pay_cents: 17250,
            credit_applied_cents: 0,
            line_items: [],
          },
          error: null,
      ...
      fireEvent.click(screen.getByText('Clear End Time'));
    ...
    it('keeps fallback pricing fields when refreshed booking details omit or invalidate them', async () => {
      const { fetchBookingDetails } = jest.requireMock('@/src/api/services/bookings') as {
        fetchBookingDetails: jest.Mock;
      };
      ...
      await waitFor(() => {
        expect(fetchBookingDetails).toHaveBeenCalledWith('booking-123');
        expect(latestPaymentConfirmationProps?.booking).toMatchObject({
          basePrice: 100,
          totalAmount: 115,
          duration: 60,
    ...
    it('renders confirmation flow cleanly in production mode', async () => {
  ...
  describe('targeted branch coverage', () => {
    it('derives preview duration from end time when metadata duration is an invalid string', async () => {
      const requestPricingPreview = jest.fn();
      usePaymentFlowMock.mockReturnValue({
        currentStep: PaymentStep.CONFIRMATION,
        paymentMethod: PaymentMethod.CREDIT_CARD,
      ...
      render(
        <PaymentSection
          ...
          bookingData={{
            ...mockBookingData,
            duration: undefined as unknown as number,
            metadata: { serviceId: 'service-789', duration: 'not-a-number' },
          }}
        />,
        { wrapper: createWrapper() }
      );
    ...
    it('skips credit-storage persistence branches when quote selection is unavailable', async () => {
      const applyCredit = jest.fn().mockResolvedValue({ credit_applied_cents: 2500 });
      const selectPaymentMethod = jest.fn();
    ...
    it('falls back to generic handling when checkout rejection has no message field', async () => {
      const goToStep = jest.fn();
      const onError = jest.fn();
      const selectPaymentMethod = jest.fn();
      ...
      paymentServiceMock.createCheckout.mockRejectedValue({ code: 'missing-message' });
      ...
      fireEvent.click(screen.getByText('Select Card'));
      await waitFor(() => {
        expect(selectPaymentMethod).toHaveBeenLastCalledWith('CREDIT_CARD', 'card-1', undefined);
      });
      fireEvent.click(screen.getByText('Confirm Payment'));
      ...
      expect(goToStep).toHaveBeenCalledWith(PaymentStep.ERROR);
      expect(onError).toHaveBeenCalledWith({ code: 'missing-message' });
    });

    it('renders success without card metadata for credits-only completions', async () => {
      usePaymentFlowMock.mockReturnValue({
        currentStep: PaymentStep.SUCCESS,
        paymentMethod: PaymentMethod.CREDITS,
        creditsToUse: 115,
      ...
      expect(latestPaymentSuccessProps).toEqual({ confirmationNumber: '' });
    });
  });
});
