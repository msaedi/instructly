...
import { generateAvailabilityOperations, validateOperations } from '@/lib/availability/operationGenerator';
...
export function useAvailabilityOperations(deps: {
   ...
   * Save week schedule
   */
  const saveWeekSchedule = useCallback(async (
    options: SaveOptions = {}
  ): Promise<SaveResult> => {
   ...
   * Apply schedule to future weeks
   */
  const applyToFutureWeeks = useCallback(async (
    endDate: string
  ): Promise<ApplyResult> => {
    ...
    try {
      ...
      const hasChanges = Object.keys(weekSchedule).length !== Object.keys(savedWeekSchedule).length ||
      ...
      return {
        success: true,
        message: `Schedule applied to future weeks through ${new Date(endDate).toLocaleDateString()}`,
        slotsCreated: result.slots_created,
