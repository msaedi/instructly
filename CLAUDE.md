...
**Rules:**
- Never use `parseInt()` on IDs — they're ULID strings
- Never compare IDs numerically — use string comparison
- Never use `number` type for IDs in TypeScript — always `string`
- Always 26 characters, generated by application (not database)

## Architecture

### Backend (FastAPI) — 5 Layers
1. **Routes**: HTTP endpoints → `backend/app/routes/`
2. **Services**: Business logic with DI → `backend/app/services/`
3. **Repositories**: Data access abstraction → `backend/app/repositories/`
4. **Models**: Database schemas → `backend/app/models/`
5. **Schemas**: Pydantic validation → `backend/app/schemas/`

**Backend patterns (enforced):**
- All services extend `BaseService`
- Use `@measure_operation` decorator on public methods
- Repository pattern: `service.repository.method()` — never direct DB queries in services
- Transaction pattern: `with self.transaction():` — never `db.commit()`
- Dependency injection everywhere — no singletons

### Frontend (Next.js 16) — Flat Structure (NO `src/` directory)
- `app/` — Page routing and layouts (App Router)
- `components/` — Reusable UI components
- `features/` — Feature-specific code (includes API services, hooks)
...
### TypeScript (Strictest Possible — Zero Errors)
```json
{
  "strict": true,
...
# Strict mode (STRICT_SCHEMAS=1): extra='forbid' → 422 on unknown fields
...
**ALL API routes are under `/api/v1/*`** — no exceptions (auth, admin, health, metrics — everything except `/docs`, `/redoc`, `/openapi.json`).

### OpenAPI → TypeScript Contract Enforcement
...
|------|---------|----------|-------|
| **INT** 🟢 | *(default — no flag needed)* | `instainstru_test` | pytest, scripts |
| **STG** 🟡 | `USE_STG_DATABASE=true` | `instainstru_stg` | Local dev (`./run_backend.py` uses this) |
| **PROD** 🔴 | `USE_PROD_DATABASE=true` | Supabase PostgreSQL | Requires interactive confirmation |

`settings.database_url` defaults to INT — safe by default.
...
**Modify existing migration files** in `backend/alembic/versions/` — do NOT create new ones with `alembic revision`. Reset and rebuild with `prep_db.py` since no production data exists.

## 🔧 CI Database Image

Custom PostgreSQL: `ghcr.io/msaedi/instructly-ci-postgres:14-postgis-pgvector`
- Source: `.github/docker/postgres-ci/Dockerfile`
- Required: both PostGIS AND pgvector — migrations fail without them
- Do NOT use standard postgres image or random community images

## Client-Side Caching: React Query (Mandatory)

ALL data fetching uses React Query (TanStack Query v5) — never raw `fetch()` or `useEffect` for API calls.
...
ZIP code → timezone mapping during registration:
...
API, Redis ($7/mo), Celery Worker, Celery Beat, Flower, Supabase PostgreSQL — ~$60/month total. Assets via Cloudflare R2 (`assets.instainstru.com`).
...
A-Team designs in `docs/a-team-deliverables/` — ASCII mockups are the official designs.
