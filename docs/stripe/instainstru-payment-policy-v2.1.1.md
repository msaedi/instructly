...
*This document supersedes v2.1 and all prior payment policy documents.*
...
**If immediate authorization fails at checkout:**
- The booking must remain **unconfirmed** (and must not reserve instructor time).
- The student may retry with a different payment method (including completing any required SCA/3DS challenge).
- If payment is not successfully authorized within **30 minutes**, the booking is automatically canceled.
...
Rescheduling applies only to changing the lesson time with the **same instructor**. Changing instructors requires canceling the existing booking and creating a new booking; cancellation rules apply to the original booking.
...
Set:
- `payment_status = settled`
- `settlement_outcome = instructor_no_show_full_refund`

### 8.2.1 No-show reporting and adjudication
A no-show must be explicitly reported and resolved (it is not automatic).

**Who can report:**
- Student may report instructor no-show.
- Admin/support may also mark instructor no-show.

**Reporting window:**
- From `lesson_start_at` until `lesson_end_at + 24h`.

**Immediate payment freeze (required):**
- When a no-show report is created, set `payment_status = manual_review` with reason `instructor_no_show_reported`.
- This booking is considered “in dispute” for Section 5.3; capture/payout automation must not run while in `manual_review`.

**Instructor response window:**
- Instructor may dispute the no-show report within 24h of `no_show_reported_at`.

**Resolution:**
- If undisputed after 24h: auto-confirm and apply the instructor no-show outcome in Section 8.2.
- If disputed: platform reviews and determines outcome:
  - If student wins: apply Section 8.2.
  - If instructor wins: proceed with normal settlement (capture/payout) as soon as possible; if the original authorization is no longer capturable, re-authorize and capture (per Section 10.2).

**Audit trail required:**
- Record `no_show_reported_at`, `no_show_reported_by_user_id`, `no_show_disputed_at` (optional), `resolution_at`, `resolution_by`, `resolution_reason`.
...
**Credit expiration while reserved:**
Credits reserved for a booking are protected from expiration until the booking is settled. Expiration is evaluated at the time of reservation.
...
- Credits reserved for a booking do not expire while reserved; expiration is evaluated at the time of reservation.
- If reserved credits are released back to the student, they retain their original expiration timestamp.
- Newly issued credits (from cancellations or adjustments) expire 1 year from issuance.
...
## 15. Change Log (v2.1 → v2.1.1)

- **No-show process added:** defined reporting window, immediate payment freeze, instructor dispute window, and resolution requirements.
- **Added no-show settlement outcome:** `instructor_no_show_full_refund`.
- **Credits clarified:** reserved credits are protected from expiration; released credits retain original expiration.
- **Clarifications added:**
  - Immediate authorization failure at checkout (retry rules + auto-cancel after 30 minutes).
  - Reschedule applies only to time changes with the same instructor; changing instructors requires cancel + rebook.
